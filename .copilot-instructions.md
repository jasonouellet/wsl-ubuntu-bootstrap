...
- **Solution**: Use `state: present` instead of `state: absent`, ensures idempotency

**Issue**: Shell command marked with changed_when: false fails
- **Solution**: Separate read-only commands from modifying commands; use command instead of shell

**Issue**: Pre-commit hooks fail on commit
- **Solution**: Run `pre-commit run --all-files` to fix issues, then commit
